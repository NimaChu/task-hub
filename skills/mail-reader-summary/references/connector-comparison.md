...
IMAP itself carries no contacts. A 2026-09-08 test of the NetEase CardDAV route found no usable company directory: the IMAP authorization code authenticated, but `REPORT addressbook-query` and vCard `GET` returned 501, only a personal address book appeared, and no shared enterprise collection was reachable. Treat this as a dated provider observation, not a universal CardDAV claim. Organization data needs a verified admin LDAP/REST source or an already connected company-directory adapter such as WeCom; do not keep retrying mailbox contacts without suitable access.
