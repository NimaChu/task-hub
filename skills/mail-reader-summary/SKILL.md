...
On the first sync, default behavior is to establish a baseline at the current highest UID without importing historical messages. Only import existing mail when the project config explicitly sets `import_existing_on_first_sync` to `true` before that first sync.

For a bounded first-run test, `--initial-latest N` imports only the latest `N` existing messages and establishes the cursor after them. Use it only before a source baseline exists. It does not replay older mail later.
